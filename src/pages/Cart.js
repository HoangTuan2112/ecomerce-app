...
import { checkOut, deleteFromCart, updateCartItem } from '../features/cartSlice'
                    ...
                    <div className='col-12'>
                        ...
                        {cart.map((item) => (
                            ...
                            <div className='cart-col-1 d-flex align-items-center gap-30'>
                                <div className='w-25'>
                                    <img src={item.img} alt='product' width={"78px"} height={"130px"} />
                                </div>
                            ...
                            <div className='cart-col-3 d-flex align-items-center gap-30'>
                                <div className='d-flex align-items-center gap-10'>
                                   <BiArrowToBottom onClick={() => dispatch(updateCartItem({id: item.id, flag: 0}))} />
                                   <span>{item.count}</span>
                                      <BiArrowToTop onClick={() => dispatch(updateCartItem({id: item.id, flag: 1}))} />

                                </div>
                                <div>
                                    <AiFillDelete onClick={()=>dispatch(deleteFromCart(item.id))}/>
                                </div>
                            </div>
                        ...
                        <div className='cart-footer py-3 d-flex justify-content-end align-items-center'>
                            <button className='button' onClick={()=>{dispatch(checkOut(total))}}>Checkout</button>
                        </div>
                    </div>
                </div>
            </div>
